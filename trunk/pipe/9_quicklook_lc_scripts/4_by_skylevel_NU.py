"""
We color points according to skylevel.
"""
...
if savefigs:
	if plotnormfieldname:
		plotfilepath = os.path.join(plotdir, "%s_lc_%s_by_skylevel.pdf" % (deckey, plotnormfieldname))
	else :
		plotfilepath = os.path.join(plotdir, "%s_lc_by_skylevel.pdf" % (deckey))
