...
from src import _1_get_sky, set_mask, _2_get_stars, _3_fitmof
from src import _4_fitgaus, _5_PSFgen, deconv_simult, get_ini_par
import src._4b_fitnum_interp as _4b_fitnum
...
class MCS_interface():
    ...
    def __init__(self, cfg_file='config.py', as_exe=False):
        ...
        self.needed_par = ['FILENAME', 'STARS', 'NPIX', 'OBJ_POS', 'OBJ_SIZE', 'IMG_GAIN', 'SKY_BACKGROUND', 
                           ...
                           'FFT_DIV', 'PSF_RAD', 'MIN_STEP_NUM', 'MAX_STEP_NUM']
    ...
    def check_workspace(self):
        ...
        while not self.check_cfg_val():
            ...
            _ = raw_input()
        files, cat = ws.get_files(FILENAME, directory='images', as_exe=self.as_exe) #@UndefinedVariable
...
#        self.params = ws.pick()
    ...
    def get_stars(self, interactive=False):
        ...
        dir = 'images'
        if self.as_exe: dir = ''
        files, cat = ws.get_files(self.params['FILENAME'], directory=dir)
