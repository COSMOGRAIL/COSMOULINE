...
def getfilenames(nbsets=1, as_exe=False):
    stars, cat = get_files('star*.fits', excl = 'sig', as_exe=as_exe)
    stars= sorted(set(stars))
    sigs = sorted(set(get_files('starsig*.fits', as_exe=as_exe)[0]))
    objs = sorted(set(get_files('g*.fits', excl='sig', as_exe=as_exe)[0]))
    objsig = sorted(set(get_files('gsig*.fits', as_exe=as_exe)[0]))
    psfs = sorted(set(get_files('psf_*.fits', as_exe=as_exe)[0]))
    if as_exe:
        res_dir = ''
    if nbsets >1:
...
def restore(stars=None, sigs=None, objs=None, objssig=None, psfs=None, as_exe=False):
