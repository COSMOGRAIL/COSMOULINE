...
def clean_workspace():
    cd = os.path.join(os.getcwd(),res_dir)
    ls = os.listdir(cd)
    for e in ls:
        if os.path.isfile(os.path.join(cd, e)) and '.fits' in e:
            out(3, 'deleting '+e)
...
def backup_workspace(cfgfile, name=None):
    import shutil
    cd = os.path.join(os.getcwd(), res_dir)
    ls = os.listdir(cd)
    files = [e for e in ls
...
def get_files(filename, directory=None, excl=None, standalone=False):
    import string
    cd = directory
    if cd is None:
...
def populate(filename, spos, npix, opos, orad, sky, sig, gain, clean=0, standalone=False):
    #TODO: add debug info
    ...
    for i, f in enumerate(files):
        ...
        if sig is not None and gain is not None:
            ...
            name = 'gsig'+cat[i]+ '.fits'
            fn.array2fits(s, os.path.join(cd,name))
    return len(cat)
        

def getfilenames(nbsets=1, standalone=False):
    stars, cat = get_files('star*.fits', excl = 'sig', standalone=standalone)
    stars= sorted(set(stars))
    sigs = sorted(set(get_files('starsig*.fits', standalone=standalone)[0]))
    objs = sorted(set(get_files('g*.fits', excl='sig', standalone=standalone)[0]))
    objsig = sorted(set(get_files('gsig*.fits', standalone=standalone)[0]))
    psfs = sorted(set(get_files('s_*.fits', standalone=standalone)[0]))
    if standalone:
        res_dir = ''
    if nbsets >1:
        ...
        objs = [objs[i*onb : i*onb+1] 
                for i in range(nbsets)]
        objsig = [objsig[i*onb : i*onb+1] 
                  for i in range(nbsets)]
        psfs = [psfs[i*onb : i*onb+1] 
                  for i in range(nbsets)]
    return stars, sigs, objs, objsig, psfs

def get_filenb(filename):
    if type(filename) == type([]):
        return len(filename)
    return len(get_multiplefiles(filename, directory='images')[1])


def restore(stars=None, sigs=None, objs=None, objssig=None, psfs=None, standalone=False):
    data = {'stars':None, 'sigs':None, 'objs':None, 'objssig':None, 'psfs':None}
    if standalone:
        cd = ''
        res_dir = ''
    else:
