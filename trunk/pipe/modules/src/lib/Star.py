...
class Star():
    ...
    def __init__(self, id, image, param, sampling_factor, final_fwhm, nm_mod, use_mom=False):
        ...
        self.diffm              = Image(zeros(self.bshape  , dtype = float64))
        self.diffg              = Image(zeros(self.bshape  , dtype = float64))
        self.diffnum            = Image(zeros(self.bshape  , dtype = float64))
        #the errors of each method:
        self.chi                = zeros(self.bshape, dtype = float64)
        self.chig               = zeros(self.bshape, dtype = float64)
        self.chinum             = zeros(self.bshape, dtype = float64)
        #copy of the residuals, used to allow iterative use of the gaussian technique  
        self.residuals          = zeros(self.bshape, dtype = float64)
        #gaussian used in an eventual convolution
        self._gaussian          = fromfunction(self._gaus(0, 2, 0, self.width/2.-1, self.height/2.-1, 1),
                                              self.sshape)
    ...
    def moff_eval(self):
        ...
        """
        self.psfm.setMof(append(self.par.getMofpar(0), self.par.getLocpar(self.id-1)), fwhm0 = self.wr)
        means = self._mean(self.psfm.array, self.bshape[0], self.bshape[1])
        self.chi = (self.image.array - means)/self.image.noiseMap
    ...
    def build_diffm(self):
        ...
        if self.nm_mod == True:
            ...
            self.image.noiseMap *= fromfunction(lambda x,y: sqrt((x-c1)**2.+(y-c2)**2.)+1, self.bshape)
    ...
    def gpos_eval(self, params):
        ...
        par = array(params).reshape(params.shape[0]/2, 2)
...
#                 ['/factor[%d]'%i for i in xrange(lenShape)]
    ...
    def _gaus(self, theta, fwhm, e, c1, c2, i0):
        ...
        sigx = sqrt((fwhm / (2. * sqrt(2.*log(2.))))**2 + (self.wr / (2. * sqrt(2.*log(2.))))**2)
        sigy = sigx /(1. - e**2)
