...
def fitnum(fit_id, data, params, savedir = 'results/'):
    ...
    minstep, maxstep = params['MIN_STEP_NUM'], params['MAX_STEP_NUM']
...
#    bak = dec.model
    ...
    for i, im in enumerate(stars):           
        gaus_err += abs(dec.get_im_resi(bak_conv, i)[0]).sum()
    ...
    names += ['psf', 's', 'psf_num', 'psfn_ini']
    for i, s in enumerate(stars):           
        resi = dec.get_im_resi(bak_conv, i)[0]
        imgs += [s.diffm.array, resi]
        names += ["difmof%(fnb)02d_%(n)02d.fits" % {'fnb':fit_id+1, 'n': s.id}, 
                  "difnum%(fnb)02d_%(n)02d.fits" % {'fnb':fit_id+1, 'n': s.id}]
            
    ############ Save and display ############
    if savedir is not None:
        out(2, 'Writing to disk...')
        for i, im in enumerate(imgs):
            fn.array2fits(im, savedir+names[i])
