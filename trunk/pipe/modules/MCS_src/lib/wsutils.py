...
def get_files(filename, directory=None, excl=None):
    import string
    cd = directory
    if cd is None:
        cd = os.path.join(os.getcwd(),'results')
...
def populate(filename, spos, npix, opos, orad, sky, sig, gain, clean=0):
    #TODO: add debug info
    ...
    if '*' in filename:
        files, cat = get_multiplefiles(filename, cdim)
    else:
        files, cat = [filename], ['']
...
def getfilenames(nbsets=1):
    stars, cat = get_files('star*.fits', excl = 'sig')
    stars= sorted(set(stars))
    sigs = sorted(set(get_files('starsig*.fits')[0]))
    objs = sorted(set(get_files('g*.fits', excl='sig')[0]))
    objsig = sorted(set(get_files('gsig*.fits')[0]))
    psfs = sorted(set(get_files('s_*.fits')[0]))
    if nbsets >1:
...
def restore(stars=None, sigs=None, objs=None, objssig=None, psfs=None):
