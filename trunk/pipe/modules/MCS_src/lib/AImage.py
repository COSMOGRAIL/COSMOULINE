...
class Image:
    ...
    def __init__(self, numpyarray=None, shape = (0,0), noisemap=None):
        """
        ...
        if numpyarray == None:
            self.array = zeros(shape, dtype=float64)
        else:
            if not isinstance(numpyarray, ndarray):
            ...
            self.array = numpyarray.astype(float64)
            self.setzscale()
    ...
    def showmatplot(self):
        ...
        plt.matshow(self.array.transpose(), vmin=self.z1, vmax=self.z2, cmap=cm.hot, origin="lower", extent=(0, self.array.shape[0], 0, self.array.shape[1]))
        if self.noiseMap != None:
            plt.matshow(self.noiseMap.transpose(), vmin=self.nm_z1, vmax=self.nm_z2, cmap=cm.hot, origin="lower", extent=(0, self.array.shape[0], 0, self.array.shape[1]))
    ...
    def readfromfits(self, filename):
        ...
        self.array = asarray(self.array, dtype=float64)
    ...
    def extractfromfits(self, filename, loc, size, sky=0.):
        ...
        if x+radius+r >= fulldata.shape[1] or y+radius+r >= fulldata.shape[0] or x-radius<0 or y-radius<0:
            ...
            print "outside"
        self.array = self.array + zeros(self.array.shape, dtype=float64)    # switch to 8 byte   
        self.array = fulldata[y-radius:y+radius+r, x-radius:x+radius+r].transpose()            # get values from the subsection 
        #    This tansposition makes the pixelarray coordinates (x,y) equal to those in the ds9 display etc.
        #    In other words, we are in the math and astro convention.
        #    x = horizontal, y = vertical, (0, 0) is bottom left.
        self.array[where(isnan(self.array))] = sky
        hdulist.close()
        if sky is None:
            #TODO: check...
            self.array -= stats.mode(data.ravel())[0][0]
        else:
            self.array -= sky
        self.setzscale()
    ...
    def normalize(self):
        ...
        if self.noiseMap != None:
            ...
            for index, pix in ndenumerate(self.noiseMap):
    ...
    def buildNoiseMap(self, e_adu, std_deviation):
        ...
        self.noiseMap = sqrt(abs(self.array)/e_adu + std_deviation**2)
    ...
    def setzscale(self, mode = 'full', z1='0', z2='1'):
        if mode == 'full':
            ...
            sky = median(self.array[[0, 1, -2, -1],:].flatten())
    ...
    def getCenter(self, absolute=False, center_mode='SW', middle=False):
        ...
        sumIy = 0
        sumI = 0
        for index, val in ndenumerate(self.array):
            sumI  += val
            sumIx += val * index[0]
            sumIy += val * index[1]
        c1 = (sumIx / sumI)
    ...
    def shiftToCenter(self, c1, c2, interp_order = 3, center_mode = 'SW', ret = False):
        """
        ...
        offY = c2 - y0
        offX = c1 - x0
        indexes = indices(self.array.shape)
        xrange = indexes[0] + offX
        yrange = indexes[1] + offY
        #get the positions for each pixel in the new system:
        new_pos = array([ravel(xrange), ravel(yrange)])
        #shift:
        s = self.array.sum()
        t = ndimage.map_coordinates(self.array, new_pos, order=interp_order, mode='wrap')
        #reshape:
        t.shape = w,h
        t *= s/t.sum()
        if ret is False:
