...
c     ===========================================
      write(*,*) maskmodel,nxy2,cxmask,cymask,raymask
      if (maskmodel .eq. 1) then
         do j=1,nxy2
            ind2 = (j-1)*nxy2
...
c     - - - - - - - - - - - - - -
      ...
      write(*,*) 
      if (amodel .eq. 1) then
         write(*,*) 'DECONVOLUTION AVEC MODELE ANALYTIQUE'
      else
         write(*,*) 'DECONVOLUTION SANS MODELE ANALYTIQUE'        
      end if
      write(*,*)
      write(*,*) '============ Valeurs initiales ================'
      do k=1,np
         nda(k) = a(k,1)*k1/(100.)
      ...
      do j=1,m
         ...
         write(*,*) 'z1 z2 : ',z(1,j),z(2,j)
         write(*,*) 'delta : ',delta(1,j),delta(2,j)
      end do
      if (amodel .eq. 1) then
         ...
         write(*,*) 'Modèle Io    : ',manaly(6)
         write(*,*) 'Modèle A B C : ',manaly(1),manaly(2),manaly(3)
         write(*,*) 'Modèle Cx Cy : ',manaly(4),manaly(5)
      end if
      write(*,*) '------------------------------------------------'
      write(*,*)
...
c     - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      do its=1,itmax
         ...
         write(*,*) its,' (M avec modèle)   min qi: ',fp,qi
         if (aug .eq. 0) then
           write(*,*) '================================================'
         else
           write(*,*) '************************************************'
...
c        - - - - - - - - - - - - - - -
         do i=1,np
            ...
            write(*,'(2(E12.6,A1))') cN(1,i),' ',cN(2,i),' '
         end do

c        MODIFICATION DE Z ET DELTA 
c           NOTE: LES PARAMETRES DE L IMAGE 1 NE SONT PAS MODIFIES
c        - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
         write(*,*) '  z1  z2        : ',zN(1,1),zN(2,1)
         write(*,*) '  delta1 delta2 : ',deltaN(1,1),deltaN(2,1)
         do i=2,m
            do j=1,2
            ...
            write(*,*) '  z1  z2        : ',zN(1,i),zN(2,i)
            write(*,*) '  delta1 delta2 : ',deltaN(1,i),deltaN(2,i)
         end do
...
c        - - - - - - - - - - - - - - - 
         ...
         write(*,*) '  Modèle analytique: '
         write(*,'(A13,E12.6)')'     Io    : ',manalyN(6)
         write(*,'(A13,3(E12.6,A1))')'     A B C : ',manalyN(1),' ',
     &                               manalyN(2),' ',manalyN(3),' '
         write(*,'(A13,2(E12.6,A1))')'     Cx Cy : ',manalyN(4),' ',
     &                                   manalyN(5),' '
         write(*,*) '  f : ',pN(1),pN(512),pN(1024)
...
c           - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            aug = 1
            arret = arret + 1
            write(*,*) its+1,' (M avec model)   min qi: ',fp,qi
            ...
            do i=1,np
               write(*,'(''   a...a c1 c2   : '',$)')
               do j=1,m2
                  write(*,'(E12.6,A1,$)') a(i,j),' '
               end do
               write(*,'(2(E12.6,A1))') c(1,i),' ',c(2,i),' '
            end do

            do i=1,m
               write(*,*) '  z1  z2        : ',z(1,i),z(2,i)
               write(*,*) '  delta1 delta2 : ',delta(1,i),delta(2,i)
            end do

            write(*,*) '  Modèle analytique: '
            write(*,'(A13,E12.6)')'     Io    : ',manaly(6)
            write(*,'(A13,3(E12.6,A1))')'     A B C : ',manaly(1),' ',
     &                                  manaly(2),' ',manaly(3),' '
            write(*,'(A13,2(E12.6,A1))')'     Cx Cy : ',manaly(4),' ',
     &                                      manaly(5),' '
            write(*,*) '  f : ',p(1),p(512),p(1024)
...
c     - - - - - - - - - - - - - - - - - - - - - - - 
      ...
      do its=1,itmax
         ...
         write(*,*) its,' (M sans modèle)   min qi: ',fp,qi
         if (aug .eq. 0) then
           write(*,*) '================================================'
         else
           write(*,*) '************************************************'
...
c           - - - - - - - - - - - - - - - - - - - - - - - - - - 
            ...
            aug = 1
            arret = arret + 1
            write(*,*) its+1,' (M sans modèle)   min qi: ',fp,qi
