...
c     BOUCLE SUR LE CALCUL DU FOND DE LA PSF
c     - - - - - - - - - - - - - - - - - - - -
      do while (execute .eqv. .true.)
         do j=1,n2
            p(j) = 0.
         end do
         write(*,*)
         write(*,'(''Entrez la valeur de lambda 1: '',$)')
         read(*,*) lambda
         write(*,'(''Entrez la valeur de lambda 2: '',$)')
         read(*,*) al
         write(*,'(''Entrez la valeur du rayon: '',$)')
         read(*,*) x0
         write(*,*)
         al=log(al/lambda)
         ...
         question = 'a'
         do while (question .ne. 'y' .and. question .ne. 'n')
            write(*,*) 
            write(*,*) '============================================='
            write(*,*)
            write(*,'(''Voulez-vous REFAIRE une execution (y/n): '',$)')
            read(*,'(A1)') question
         end do
         if (question .eq. 'n') execute = .false.
      end do
...
c     - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      do while (question .ne. 'y' .and. question .ne. 'n')
         ...
         write(*,*) '============================================='
         write(*,*)
         write(*,'(''Voulez-vous utiliser un MASQUE (y/n): '',$)')
         read(*,'(A1)') question
      end do
      execute = .true.
      do while (execute .eqv. .true.)
          if (question .eq. 'y') then
            ...
            do while (question .ne. 'y' .and. question .ne. 'n')
             ...
             write(*,*) '============================================='
             write(*,*)
             write(*,'(''Voulez-vous utiliser un AUTRE masque (y/n): '',
     &               $)')
             read(*,'(A1)') question
            end do
         else
...
c     ------------------
      ...
      write(*,'(''Entrez le rayon de la zone a masquer: '',$)')
      read(*,*) X0
      eps = 0.4
