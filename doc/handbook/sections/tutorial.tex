...
\tbrem{Warning : we have now changed the pipeline so that images will be converted into electrons just after the import. This is done using pyfits. The problem : in case your input images are for instance stored as integers but contain these two conversion factors in the header so that ds9 etc convert them on the fly to floats, you should make sure that this is handled well by pyfits ! 


In other words, to be safe : use images that are directly written as floats, in ADU. For cosmouline, the gains is to be given in e-/ADU, and the readnoise is in electrons.
}
...
\tbrem{convert to electrons : makes the first hard copy of the images, keeps the original gain as origin\_gain, but changes gain to 1.0 (as the images are now in electrons), AND ALSO MULTIPLIES THE SATUR\_LEVEL, so that the satur\_level is also in electrons an can be directly used for sextractor etc.}
