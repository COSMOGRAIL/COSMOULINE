...
if sys.path[0]:
    ...
    sys.path.append(os.path.join(os.path.dirname(sys.path[0]), 'pipe'))
else:
    # if ran interactively, append the parent manually as sys.path[0] 
    # will be emtpy.
    sys.path.append('../pipe')
from lcconfig import dbfilepath, telescopenames, setnames, deconvnames, \
                     normcoeffname, imgmaxseeing, imgmaxell, imgmaxrelskylevel,\
                     nightmaxseeing, imgmaxmedcoeff, imgskiplistfilename,\
                     sourcenames, lcmanipdir, outputname, nightmaxell,\
                     nightmaxrelskylevel, nightmaxnormcoeff,\
                     nightminnbimg, writeheader, showplots
from config import imgdb
from modules.variousfct import readpickle
from modules.kirbybase import KirbyBase
from modules import groupfct, variousfct, rdbexport
...
# Reading the db :
allimages = readpickle(dbfilepath, verbose=True)
print("%i images in db." % (len(allimages)))

allimages = [image for image in allimages if (image["telescopename"] in telescopenames) 
                                            and (image["setname"] in setnames) 
                                            and (image["gogogo"] == True) ]
print("%i images have the chosen telescope- and set- names." % (len(allimages)))

for setname, deconvname in zip(setnames, deconvnames):

    
    # Selecting the right deconvolution :
    images = [image for image in allimages if image["decfilenum_" + deconvname] != None] 
    ...
    plt.show()
    fig1.savefig(os.path.join(lcmanipdir,  outputname + '_' + setname + "_median_seeing.png"))
    ...
    rdbexport.writerdb(exportcols, 
                       os.path.join(lcmanipdir, f"{outputname}_{setname}.rdb"), 
    ...
    if showplots == True:
        plt.show()
        fig2.savefig(os.path.join(lcmanipdir,  outputname + '_' + setname + "_plot.pdf"))
        print("Wrote plot.")
    else:
        fig2.savefig(os.path.join(lcmanipdir,  outputname + '_' + setname + "_plot.pdf"))
        print("Wrote plot.")
    ...
    if showplots == True:
        ...
        fig3.savefig(os.path.join(lcmanipdir, outputname + '_' + setname + "_plot_magerrs.pdf"))
        print("Wrote plot.")
