...
c     ------------------
      ...
      nxy2=xy2
      pas = nxy2/nxy1
      write(*,*) n1,n2,ns,nxy1,nxy2,pas
      if (n1 .eq. n2) then
         write(*,*) 'On reste en gros pixels'
      else if (n1*pas*pas .eq. n2) then
         write(*,*) 'On passe en petits pixels'
      else
...
c     - - - - - - - - - - - - - - - - - -
      ...
      if (minimum_moffat .eq. 'i') then
         ...
         do j=1,ni
            read(1,*) b(j,1),b(j,2),b(j,3),beta(j)
            write(*,*) b(j,1),b(j,2),b(j,3),beta(j)
...
c     ----------------------------------------------
      ...
      write(*,*) nbr1,nbr2,n1,n2,xy1,xy2,nxy1,nxy2,nbi,ni
      write(*,*) plan(1,1),plan(1,2),plan(1,3)
      write(*,*) dplan(1,1),dplan(1,2),dplan(1,3)
      write(*,*) b(1,1),b(1,2),b(1,3),beta(1)
      write(*,*) db(1,1),db(1,2),db(1,3),dbeta(1)
      write(*,*) ns
      write(*,*) bb
      do i=1,ns
         write(*,*) a(1,i),c(i,1),c(i,2)
         write(*,*) da(1,i),dc(i,1),dc(i,2)
      end do
      write(*,*) delta(1,1),delta(1,2)
      write(*,*) ddelta(1,1),ddelta(1,2)
...
c     - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      write(*,*) 'Nombre de relances de minimi ',ray,marche,nb_iter
      do i=1,nb_iter
...
c     - - - - - - - - - - - - - - - - -
      ...
      write(*,*) '-----'
      do j=1,ni
         write(*,*) j,' alpha beta gamma: ',
     &              plan(j,1),plan(j,2),plan(j,3)
      end do
      write(*,*) 
      write(*,*) 'MOFFAT:'
      write(*,*) '-------'
      do j=1,ni
         write(*,*) j,' b1 b2,b3,beta:   ',b(j,1),b(j,2),b(j,3),beta(j)
      end do
      write(*,*) 
      write(*,*) 'SOURCES:'
      write(*,*) '--------'
      do i=1,ns
         write(*,*) 'source ',i,' a c1 c2 : ',
     &              (a(j,i),j=1,ni),
     &              c(i,1)+((real(pas)-1)/real(pas)),
     &              c(i,2)+((real(pas)-1)/real(pas))
      end do
      write(*,*) 'DECALAGES:'
 1    write(*,*) '-----------'
      do i=1,ni
         write(*,*) 'décalage ',i,' :   ',delta(i,1),delta(i,2)
      end do
      write(*,*) 
      write(*,*) '==================================='
      write(*,*) 
...
c     - - - - - - - - - - - - - - - - - - - -
      ...
      do its=1,itmax
         ...
         write(*,*) its,' M  (moffat)   min khi: ',fp,qi
         write(*,*)
         iter = its
...
c        - - - - - - - - - - - - - - 
         do l=1,ni
            ...
            write(*,'(E12.6)') betaN(l)
            write(*,*) 'plan :         ',planN(l,1),planN(l,2),
     &                  planN(l,3)
         end do
...
c        - - - - - - - - - - -
         do k=1,ns
            ...
            write(*,'(2(E12.6,A1))') cN(k,1)+((real(pas)-1)/real(pas)),
     &            ' ',cN(k,2)+((real(pas)-1)/real(pas)),' '
...
c        - - - - - - - - - - - - 
         ...
         write(*,'(2(E12.6,A1))') delta(1,1),' ',delta(1,2),' '
         do l=2,ni
            write(*,'(''delta x   delta y   :'',$)')
            do i=1,2
            ...
            write(*,'(2(E12.6,A1))') delta(l,1),' ',delta(l,2),' '
         end do
...
c           - - - - - -
     ...
     &                  ddeltaN(l,i) = ddelta(l,i)*1.1
            ...
            write(*,*)
            write(*,*) '==============================================='
            write(*,*) ' ON DEPASSE LE MIN (dépassement n° ',arret,' )'
            write(*,*) '==============================================='
            write(*,*)
            if (arret .eq. 10) then
               write(*,*)
               write(*,*) '=============================='
               write(*,*) '  MINIMUM DEPASSE 10 fois...'
               write(*,*) '=============================='
               return
            end if
...
c           - - - - - -
      ...
      write(*,*) "Nombre d'iterations excédé"
...
c     - - - - - - - - - - - - - - - - - -
      ...
      do its=1,itmax
         ...
         write(*,*) its,' M  (fond)   min khi: ',fp,qi
        if (aug .eq. 0) then
         write(*,*) '=================================================='
        else
         write(*,*) '**************************************************'
...
c        - - - - - - - - - - -
     ...
     &               (dfN(ind,l)*dfgauss)
            ...
            write(*,*) 'Image ',l,
     &                 ', Nbr de pixels qui changent : ',flag 
            if (flag .eq. 0) then
               write(*,*) 'Plus rien ne change'
               goto 10
            end if
...
c        - - - - - - - - - - - - 
         ...
         deltaN(1,2) = delta(1,2)
         write(*,'('' Delta x   Delta y : '',$)')
         write(*,'(2(E12.6,A1))') deltaN(1,1),'  ',deltaN(1,2),' '
         do l=2,ni
            write(*,'('' Delta x   Delta y : '',$)')
            do i=1,2
               if(xidelta(l,i) .lt. 0.) then
            ...
            write(*,'(2(E12.6,A1))') deltaN(l,1),' ',deltaN(l,2),' '
         end do
...
c           - - -
            ...
            write(*,*) its+1,' M  (fond)   min khi: ',fp,qi
            write(*,*)
            write(*,*) '  ON DEPASSE LE MIN (dépassement n° ',arret,' )'
            write(*,*) '==============================================='
            write(*,*) '******* ON REPART DES VALEURS SUIVANTES *******'
            do k=1,ns
               write(*,'(''a ... a cx cy       :'',$)')
               do l=1,ni
                  write(*,'(E12.6,A1,$)') a(l,k),' '
               end do
               write(*,'(2(E12.6,A1))') c(k,1),' ',c(k,2),' '
            end do
            do l=1,ni
               write(*,'(''delta x   delta y   :'',$)')
               write(*,'(2(E12.6,A1))') delta(l,1),' ',delta(l,2),' '
            ...
            do l=1,ni
               write(*,*) 'plan =  ',
     &           plan(l,3),' + ',plan(l,1),' x + ',plan(l,2),' y + '
            end do
...
c     *************************************************
      ...
      do l=1,ni
         ...
         write(*,*) 'Maxlissage ',l,' : ',maxlissage(l)
      end do
      ...
      do its=1,100
         ...
         write(*,*) its,' M  (cycle 2)   min khi lis: ',fp,qi,lis
        if (aug .eq. 0) then
         write(*,*) '=================================================='
        else
         write(*,*) '**************************************************'
...
c           - - -
            ...
            write(*,*)
            write(*,*) '  ON DEPASSE LE LISSAGE (dépas. n° ',arret,' )'
            write(*,*) '==============================================='
            write(*,*) '******* ON REPART DES VALEURS SUIVANTES *******'
            do k=1,ns
               write(*,'(''a ... a cx cy       :'',$)')
...
c        -------------
         ...
         if ((recompose .eq. 'y') .and. (ns .ne. 1)) then
            ...
            fin = nxy2/2 + nxy2/4
            taille = int(nxy2/2)
            write(*,*) 'DECOUPAGE DE S SUITE A LA RECONSTRUCTION'
         else 
            debut = 1
            fin = nxy2
            taille = nxy2
