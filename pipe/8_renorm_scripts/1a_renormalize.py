...
for telescopename in telescopenames:
    ...
    for j, renormsource in enumerate(renormsources):
        ...
        medflux = np.nanmedian(fluxes)  # This is the plain simple unnormalized ref flux that we will use for this star.
    ...
    for i, image in enumerate(telimages):
        ...
        if len(image["tmp_indivcoeffs"]) == 0:
            ...
            renormcoeff = np.nanmedian(np.array(image["tmp_indivcoeffs"]))  # median of multiplicative factors
            renormcoefferr = np.nanstd(np.array(image["tmp_indivcoeffs"]))
...
for i, image in enumerate(allimages):
    ...
    db.update(imgdb, ['recno'], [image['recno']],
              {renormname: float(image["renormcoeff"]), renormerrfieldname: float(image["renormcoefferr"]),
