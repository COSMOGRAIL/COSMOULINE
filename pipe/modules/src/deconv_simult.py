...
def deconv(data, params):
    ...
    if cuda and fn.has_cuda():
        ...
        psf = [fn.switch_psf_shape(p, 'SW') for p in psf]
    else:
        convol = fn.conv
        div = None#fn.div
        r = fn.gaussian((r_len, r_len), gres, c1, c2, 1.)
...
#        lamb += [10.*abs(obji[i])/stddev[i]]
        ratio = abs(obj[i]/sigma[i])
#        ratio = abs(obj[i])/stddev[i]
#        ratio /= median(ratio)
        ratio = minimum(ratio, 200.).astype('float64')
        ratio = maximum(ratio, 1.)/10.
#        ratio = sig.wiener(abs(ratio), (3,3))
        lamb += [log(ratio)**2.]
#        lamb += [ratio]
