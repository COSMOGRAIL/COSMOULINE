...
#        err += khi_smooth
        for i, s in enumerate(STAR_COL):           
            bk = Image(param) 
            bk.shiftToCenter(r_len-mpar[fit_id][4+3*i],r_len-mpar[fit_id][4+3*i+1], 
                             interp_order = 3, center_mode='O', mode='reflect')
...
#            if i == 0: fn.array2ds9(dif.array)
    ...
    fn.array2ds9(_errfun(bak, []).reshape(sshape), name='chi2', frame=5)#, zscale=True)
...
#            resi = (s.diffm.array-fn.mean(conv(r,bak), bshape[0], bshape[1]))/s.image.noiseMap
    ...
    if show == True:
        ...
        for j, s in enumerate(STAR_COL):           
            ...
            bk = Image(bak) 
            bk.shiftToCenter(r_len-mpar[fit_id][4+3*j],r_len-mpar[fit_id][4+3*j+1], 
                             interp_order = 1, center_mode='O', mode='reflect')
            convo = conv(r, bk.array)
            convo_m = fn.mean(convo, bshape[0], bshape[1])
#            resi = ((s.diffm.array - convo_m)/s.image.noiseMap)**2.
...
#            dif = Image(resi) 
#            dif.array = fn.rebin(dif.array, sshape)/sfactor**2.
#            dif.shiftToCenter(mpar[fit_id][4+3*j],mpar[fit_id][4+3*j+1], 
#                              interp_order = 1, center_mode='O', mode='reflect')
#            resi = dif.array
            fn.array2ds9(resi, frame=i+1, name="resi_"+str(s.id))
            i += 2
#        fn.array2ds9(_errfun(bak, []).reshape(sshape), name='chi2', frame=i)#, zscale=True)
    import pylab as p
    p.figure(1)
    trace = array(TRACE)
