...
class Image:
    ...
    def showmatplot(self):
        ...
        plt.matshow(self.array.transpose(), vmin=self.z1, vmax=self.z2, cmap=cm.hot, origin="lower",
                    extent=(0, self.array.shape[0], 0, self.array.shape[1]))  # @UndefinedVariable
        if self.noiseMap != None:
            plt.matshow(self.noiseMap.transpose(), vmin=self.nm_z1, vmax=self.nm_z2, cmap=cm.hot, origin="lower",
                        extent=(0, self.array.shape[0], 0, self.array.shape[1]))  # @UndefinedVariable
    ...
    def writetofitsNM(self, filename):
        if self.noiseMap != None:
            ...
            raise NameError('No noise map built')
    ...
    def extractfromfits(self, filename, loc, size, sky=0.):
        ...
        if x + radius + r >= fulldata.shape[1] or y + radius + r >= fulldata.shape[
            0] or x - radius < 0 or y - radius < 0:
            # TODO: set outside pixels to NaN
            print("outside")
        self.array = self.array + np.zeros(self.array.shape, dtype=np.float64)  # switch to 8 byte
        self.array = fulldata[y - radius:y + radius + r,
                     x - radius:x + radius + r].transpose()  # get values from the subsection
        #    This tansposition makes the pixelarray coordinates (x,y) equal to those in the ds9 display etc.
        #    In other words, we are in the math and astro convention.
        #    x = horizontal, y = vertical, (0, 0) is bottom left.
