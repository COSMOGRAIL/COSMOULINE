...
class GausAlg():
    ...
    def __init__(self, stars, sfact, npix, mpar, std_dev=0.):
        ...
        self.bc2    = npix/2.
        self.run_nb = 0
        self.psf    = PSF((npix*sfact,npix*sfact), (npix*sfact/2.,npix*sfact/2.))
        if mpar is not None:
            self.pos = zeros((len(stars), 2))
    ...
    def pos_fun(self, strat, stars, params, fitrad):
        ...
        if strat == 'alternate':
            ...
            gpos, gnb = getGpos(stars[mainstar_id].residuals, gnb, gsize, bounds)
            ...
            gpos, gnb = getGpos(median, gnb, gsize, bounds)
            gpos[:,0] -= self.npix/2.
            gpos[:,1] -= self.npix/2.
        elif strat == 'mixed':
            gpos = array([])
            ...
            for star in stars:
                ...
                g, n = getGpos(star.residuals, gnb, gsize, bounds, gpos)
...
#            return array([]), array([]), 0., PSF((self.npix*self.sfact, self.npix*self.sfact)), zeros((self.npix, self.npix))
        ...
        ################ mini ##################
        t = time.time()
        opar = alg.minimalize(self.pos_err, array(par), itnb)
        out(2)
...
#        print opar
        ########################################
        cnt = 0
        gausdistr = zeros((self.npix, self.npix))
    ...
    def pos_err(self, params):
        res = array([])
        foreach(lambda i: self.stars[i].gpos_eval(params), arange(len(self.stars)), threads=(len(self.stars)))
        for star in self.stars:
    ...
    def twogrids_fun(self, stars, params):
        ...
        for i in xrange(0, int(outer), odispersion):
            for j in xrange(0, int(outer), odispersion):
                ...
                #par += [0.1, smoothing*2.]
        t = time.time()
        opar = alg.minimalize(self.grid_err, array(par), itnb)
        ...
        if outer > 0:        
            for i in xrange(int(c1 - outer/2), int(c1 + outer/2 + outer%2), odispersion):
                for j in xrange(int(c2 - outer/2), int(c2 + outer/2 + outer%2), odispersion):   
                    ...
                    psfg.addGaus_fnorm(opar[cnt,1], i, j, opar[cnt,0])
                    cnt += 1        
        return opar, array([]), final_errg, psfg
        
    def grid_err(self, params):
        res = array([])
        foreach(lambda i: self.stars[i].ggrid_eval(params), arange(len(self.stars)), 
    ...
    def grid_fun(self, stars, params):
        ...
        for i in xrange(int(diameter)):
            for j in xrange(int(diameter)):
                ...
                par += [0.1, smoothing*2.]
        t = time.time()
        opar = alg.minimalize(self.center_err, array(par), itnb)
        print '\nDone in:', time.time()-t, 's'
        res = array([])
        for star in self.mcs.star_tab:
    ...
    def center_err(self, params):
        res = array([])
        foreach(lambda i: self.mcs.star_tab[i].cgaus_eval(params), arange(self.mcs.star_nb), threads=self.mcs.star_nb)
        for star in self.mcs.star_tab:
    ...
    def _gaus(self, c1, c2, i0, fwhm, fwhm0 = 0.):
        ...
        g = lambda x,y: i0*exp((-(x-c1)**2. - (y-c2)**2)/(2.*sig))
        return fromfunction(g, self.sshape)
        
    def _getMedian(self, stars, noised = True, crop = True):
        res = zeros(stars[0].image.array.shape)
        cross_noise = zeros(stars[0].image.array.shape)
        for  i, star in enumerate(stars):
            if noised == True:
...
#    maxstep = maximum(dev, abs(par))
    ...
    i = 0
    while i < itnb:
        newdir = -(actives-1)*dir + actives*sign(err)
        changedir = sign(abs(olderr) - abs(err))
        newstep = changedir*step*((changedir+1)*0.6 + (-changedir+1)/4.)
        newstep = sign(newstep)*minimum(abs(newstep), maxstep)
...
def minimi(func, param, srcpar, itnb=0, prec=None, minstep_px=None, maxstep_px=None, maxpos_step=0.5, 
               max_iratio_step=0.02, stepfact=1., nbsrc=0, nbimg=1):
    if prec <= None:
        prec = 0.#1e-15
    par = append(param, srcpar)
    srclen = len(srcpar)
    ...
    if maxstep_px is None: maxstep = abs(par)
    else: maxstep = zeros(par.shape) + maxstep_px
    
    maxstep /= stepfact
    minstep = zeros(maxstep.shape) + minstep_px#/100.
    maxstep = maximum(maxstep, minstep)
    step = maxstep.copy()
    l = len(param)
    if  l > 0:
...
#            step[l+i*3:l+i*3+2] = maxstep[l+i*3+2]
    i = 0
    while i < itnb:
        direction = sign(abs(olderr) - abs(err))
        t1 = abs(olderr)
        t2 = abs(err)
        t3 = t1-t2
        step = direction*step*((direction+1)*0.55 + (-direction+1)/4.) #if no change: *2*nb else *-2/nb 
        step = sign(step)*minimum(abs(step), maxstep)
        step = sign(step)*maximum(abs(step), minstep)
        newpar = par + step
#        print step[64*128+64], newpar[64*128+64], err[64*128+64]
        olderr = err.copy()
        err = func(newpar[:-srclen], newpar[-srclen:])
        neweps = err.sum() / err.shape[0]**2.
