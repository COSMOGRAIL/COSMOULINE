...
def switch_psf_shape(psf, center='NE'):
    nx, ny = psf.shape
    ac1, ac2 = nx/2., ny/2.
    dx = dy = 0.
    if center == 'O':
...
def psf_gen(size, bak, mpar, gpar, gpos, gstrat, center, sfact=2.):
    import PSF
    center = 'SW'
    nx, ny = size
    if center == 'O':
    ...
    psf.add_source(mpar, gpar, gpos, gstrat, (c1+1./sfact, c2+1./sfact, 1.), center=center)
    if size != bak.shape:
...
def cuda_conv(plan, kernel, img):
    d1 = gpuarray.to_gpu(kernel.astype(complex64))
    d2 = gpuarray.to_gpu(img.astype(complex64))
    plan.execute(d1)
    plan.execute(d2)
    r = d1*d2
...
#        g = lambda x,y: i0*norm*exp((-(x-c1)**2. - (y-c2)**2.)/(2.*(sig_2+sig0_2)))
#        return fromfunction(g, shape)
    
        indexes = indices(shape)
        k = 2. * sqrt(2.*log(2.))
        sig_2 =  (fwhm  / k)**2.
        sig0_2 = (fwhm0 / k)**2.
        norm = sig_2/(sig_2+sig0_2)
        g = exp((-(indexes[0]-c1)**2. - 
                  (indexes[1]-c2)**2)/(2.*(sig_2+sig0_2)))
        return i0*g*norm
...
#        sig0_2 = (fwhm0 / k)**2.
#        norm = sig_2/(sig_2+sig0_2)
#        b1, e1 = int(c1-2.*fwhm), int(c1+2.*fwhm)
#        b2, e2 = int(c2-2.*fwhm), int(c2+2.*fwhm)
#        for i in xrange(b1, e2):
#            for j in xrange(b2,e2):
#                g[i,j] = exp((-(i-c1)**2. - 
#                               (j-c2)**2)/(2.*(sig_2+sig0_2)))
#        return i0*g*norm
...
def gaus_ell(shape, fwhm0, theta, fwhm, e, c1, c2, i0):
    ...
    sigy = sigx /(1. - e**2)
    g = lambda x,y: i0*exp(-(xm(x,y)/(sqrt(2.)*sigx))**2. - (ym(x,y)/(sqrt(2.)*sigy))**2.)
    return fromfunction(g, shape)
...
def makepilimage(img, ztrans="log", cutoffs=(None,None)):
    ...
    def loggray(x, a, b):
        ...
        return (log10(linval)-1.0)*0.5 * 255.0
    def lingray(x, a, b):
        return (x-float(a))/(b-a) * 255.0

    ztransarray.ravel()
    if ztrans == "log" :
        ztransarray = array(map(lambda x: loggray(x, minpixelval, maxpixelval), ztransarray))
    if ztrans == "lin" :
        ztransarray = array(map(lambda x: lingray(x, minpixelval, maxpixelval), ztransarray))
    ztransarray.shape = img.array.shape
    gvarray = zeros(img.array.shape, dtype=uint8) # an array of 256 bit gray values
...
def get_data(filename, directory=None, pos=None, transpose=True, sky = 0., retall = False): 
    ...
    data[:] += zeros(data.shape, dtype=float64)    # switch to 8 byte   
    hdulist.close()
    if pos is not None:
        x,y,size = pos
        radius = int(size/2.)
...
def getnoisemap(array, e_adu, std_deviation):
    ...
    """
    nm = sqrt(abs(array)/e_adu + std_deviation**2)
#    nm[array/nm<1.] = 1.
    return nm
...
def get_moments(data):
    ...
    return c1, c2, width_x, width_y

def get_shifts(imgs, boxsize=20.):
    nimg = len(imgs)
    ref = imgs[0]
    shifts = [[0,0]]
    c11 = maximum(0, (ref.shape[0]-boxsize)//2)
    c12 = minimum(ref.shape[0], (ref.shape[0]+boxsize)//2)
    c21 = maximum(0, (ref.shape[1]-boxsize)//2)
    c22 = minimum(ref.shape[1], (ref.shape[1]+boxsize)//2)
    for i in xrange(nimg-1):
        im = flipud(fliplr(imgs[i+1]))
