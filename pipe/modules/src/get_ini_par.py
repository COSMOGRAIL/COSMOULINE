...
def _get_ini(img, sig, psf, nb_src, gres, sfact, nimg=1, 
             src_range=None, convol=fn.conv, fft_div=None, pad=5.):
    data = fn.rebin(img, psf.shape)
    resi_bk = data.copy()
    dsig = fn.rebin(sig, psf.shape)
    refpar = 0., 0.
    if fft_div:
        out(3, 'FFT division...')
...
#        ini_bkg = dec.copy()
        ...
        def __errfun(param):
            resi = (dec - fn.gaussian(data.shape, param[0], param[1], param[2], param[3]))/dsig
...
#        resi_bk -= ini_bkg
        def __errfun(param):
            resi = (data - convol(psf,fn.gaussian(data.shape, param[0], param[1], param[2], param[3])))/dsig
            resi *= (100000.*(param[3]<0.)+1.)
            resi *= ((100. * (resi < 0.)) + 1.)
            resi *= (100000.*(abs(param[1]-refpar[0])>2. or abs(param[2]-refpar[1])>2.) + 1.)
            return (resi**2.).ravel()
...
#        psf[i] /= psf[i].sum()
    ...
    srcpos, ini = _get_ini(obji[0], sigmai[0], psf[0], nb_src, gres, sfact, nimg, 
