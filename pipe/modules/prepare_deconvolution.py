...
def prepare_deconvolution(decobjname, decname=None, decnormfieldname=None):
    ...
    for deckey, decskiplist, deckeyfilenum, setname, \
        ...
        extractedh5 = h5py.File(extracteddir / 'regions.h5', 'r')
        ...
        for i, image in enumerate(readyimages):
            ...
            print(f"PSF : {image['chosenpsf']}")
            
            # We select the normalization on the fly, 
            # as this is trivial compared to the psf selection :
            if decnormfieldname == "None":
                image["chosennormcoeff"] = 1.0    
                # in this case the user *wants* to make a 
                # deconvolution without normalization.
            else :
                image["chosennormcoeff"] = image[decnormfieldname]
                if image["chosennormcoeff"] == None:
                    print("WAAAAARRRRNIIIING: no coeff available, using 1.0!")
                    image["chosennormcoeff"] = 1.0
            print(f"Norm. coefficient : {image['chosennormcoeff']:.3f}")
                
            
    
            
            
            normcoeff = image["chosennormcoeff"]
            
            stamp = normcoeff * extractedh5[imgname]['stamps'][decobjname][()]
            stampnoise = normcoeff * extractedh5[imgname]['noise'][decobjname][()]
            ...
            if i != 0 : # that is mcsname != 0001
                db.update(imgdb, ['recno'], 
                                 ...
                                 {deckeyfilenum: decfilenum, 
                                  deckeypsfused: image['chosenpsf'], 
                                  deckeynormused: image["chosennormcoeff"]})
