...
cosmics.py is a python module to detect and clean cosmic ray hits on images (numpy arrays or FITS), using scipy, and based on Pieter van Dokkum's L.A.Cosmic algorithm.
...
See the demo scripts for example usages.
...
class cosmicsimage:
	...
	def labelmask(self, verbose = None):
		...
		if np.sum(self.mask) == 0: # no cosmics, we return an empty list :
			return []
		...
		sizes = ndimage.measurements.sum(self.mask.ravel(), labels.ravel(), range(1,n+1,1))
	...
	def findsatstars(self, verbose = None):
		...
		dilsatpixels = ndimage.morphology.binary_dilation(satpixels, structure=dilstruct, iterations=1, mask=None, output=None, border_value=0, origin=0, brute_force=False)
	...
	def lacosmiciteration(self, verbose = None):
		...
		cliped = np.clip(conved, 0.0, np.inf)
		...
		# We build a custom noise map, so to compare the laplacian to
 		...
 		m5clipped = np.clip(m5, 0.00001, np.inf) # As we will take the sqrt
		...
		f = f / noise
		f = np.clip(f, 0.01, np.inf) # as we will divide by f. like in the iraf version.
