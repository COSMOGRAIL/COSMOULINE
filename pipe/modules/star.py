...
def readmancat(mancatfilepath, verbose="True"):
    ...
    if verbose: print(("I've read %i sources from %s"%(len(table), os.path.split(mancatfilepath)[1])))
    return table
...
def formpairs(starlist1, starlist2, tolerance=2.0, onlysingle=False, transform=False, scalingratio=1.0, angle=0.0,
              shift=(0.0, 0.0), verbose=True):
    """
...
def listidentify(starlist1, starlist2, tolerance=2.0, onlysingle=False, transform=False, scalingratio=1.0, angle=0.0,
                 shift=(0.0, 0.0), verbose=True):
    """
    ...
    formpairsdict = formpairs(starlist1, starlist2, tolerance=tolerance, onlysingle=onlysingle, transform=transform,
                              scalingratio=scalingratio, angle=angle, shift=shift, verbose=verbose)
...
def findtrans(preciserefmanstars, autostars, scalingratio=1.0, tolerance=2.0, minnbrstars=5, mindist=100.0, nref=10,
              nauto=30, verbose=True):
    """
        ...
        for f, faintstar in enumerate(preciserefmanstars[:nref]):
            ...
            for bc, brightcandidate in enumerate(autostarscopy[:nauto]):
                for fc, faintcandidate in enumerate(autostarscopy[:nauto]):
                    ...
                    pairsdict = formpairs(preciserefmanstars, testcandlist, tolerance=tolerance, onlysingle=True,
                                          verbose=False)
