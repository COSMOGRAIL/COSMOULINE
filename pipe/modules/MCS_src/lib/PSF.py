...
class PSF:
    ...
    def set_finalPSF(self, mofpar, gpar, gpos, gstrat, locpar = None, center='O', fwhm0=0.):
        self.reset()
        self.add_source(mofpar, gpar, gpos, gstrat, locpar, center, fwhm0)
        self.normalize()
    
    def add_source(self, mofpar, gpar, gpos, gstrat, locpar = None, center='O', fwhm0=0., flux=None):
        c1 = c2 = None
        i0 = 1.
...
#            self.addMof(param)
    ...
    def addGaus_basic(self, fwhm, c1, c2, i0, fwhm0 = 0.):
        ...
        sig = (fwhm / (2. * sqrt(2.*log(2.))))**2. + (fwhm0 / (2. * sqrt(2.*log(2.))))**2.
        #gaussian:
        self.array[:] += i0*exp((-(self.indexes[0]-c1)**2. - (self.indexes[1]-c2)**2)/(2.*sig))
...
#        self.array += i0*tmp
    ...
    def addGaus_fnorm(self, fwhm, c1, c2, i0, fwhm0 = 0.):
        ...
        g = exp((-(self.indexes[0]-c1)**2. - 
                  (self.indexes[1]-c2)**2)/(2.*(sig_2+sig0_2)))
