...
class Param():
    ...
    def toArray(self, i = -1):
        if i == -1:
            return append(self.mofpar, append(self.gauspar, self.locpar))
        elif i < self.mofnb:
            return append(self.mofpar[i], self.locpar)
        else:
            return append(self.gauspar[i-self.mofnb], self.locpar)
    ...
    def addLocpar(self, pos, *param):
        ...
        if pos > self.locnb:
            self.locnb += 1
            self.locpar = append(self.locpar, array(param)).reshape(self.locnb, self.loclen)
        else:
            self.locpar[id-1] = array(param)
    ...
    def addMofpar(self, *param):
        ...
        self.mofpar = append(self.mofpar, array(param)).reshape(self.mofnb, self.moflen)
    
    def setMofpar(self, i, *param):
        self.mofpar[i] = array(param)
    ...
    def addGauspar(self, *param):
        ...
        self.gauspar = append(self.gauspar, array(param)).reshape(self.gausnb, self.gauslen)

    def setGauspar(self, i, *param):
        self.gauspar[i] = array(param)
    
    def getGauspar(self, i):
        return self.gauspar[i]
        
    def setOptpar(self, *param):
        self.opt_par = array(param)
        self.fromArray(array(param))
    ...
    def propose(self, *param):
        ...
        self.consnb += 1
        tab = array(param)
        self.cons = append(self.cons, tab).reshape(self.consnb, len(param))
        tot = zeros(len(param), dtype = float64)
