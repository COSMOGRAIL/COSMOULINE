...
class MCS_interface():
    ...
    def __init__(self, cfg_file='config.py', as_exe=False):
        self.cfg = cfg_file
        self.as_exe = as_exe
        self.ready = False
        self.params = {}
        ...
        self.needed_par = ['FILENAME', 'STARS', 'NPIX', 'OBJ_POS', 'OBJ_SIZE', 'IMG_GAIN', 'SKY_BACKGROUND', 
                           ...
                           'FFT_DIV', 'PSF_RAD', 'MIN_STEP_NUM', 'MAX_STEP_NUM', 'MIN_STEP_D', 'MAX_STEP_D', 
                           'WL_THRESHOLD_NUM', 'WL_THRESHOLD_DEC']
...
#            return False
    ...
    def set_up_workspace(self, extract=True, clear=False, backup=False):
        ...
        if backup:
            ...
            ws.backup_workspace(self.cfg, as_exe=self.as_exe)
        if clear:
            out(2, 'Cleaning up workspace (delete all .fits files in results/)...')
            ws.clean_workspace(as_exe=self.as_exe)
        if extract:
            self.refresh()
            ...
            ws.populate(self.params['FILENAME'], self.params['STARS'], self.params['NPIX'], 
                        ...
                        self.params['SIGMA_SKY'], self.params['IMG_GAIN'], clean = 0, as_exe=self.as_exe)
...
#        self.params = ws.pick()
    ...
    def get_sky(self):
        ...
        for i in range(fnb):
            ...
            sky, sig = _1_get_sky.get_sky_val(fn.get_data(f, directory=dir), show = self.params['SHOW'], 
    ...
    def get_stars(self, interactive=False):
        ...
        if interactive:
            ...
            stars = _2_get_stars.get_stars(img, self.params, show=self.params['SHOW'])[0]
    ...
    def put_masks(self, file='gsig.fits'):
        ...
        @param file: Name of the image to modify (sigma). Should be in the 'results' folder\
                     (default: gsig.fits).
        """
        cd = os.path.join(os.getcwd(), self.as_exe*'' or (not self.as_exe)*'results') #@UndefinedVariable
        set_mask.prompt(os.path.join(cd,file))
    ...
    def fitmof(self):
        ...
        for i in range(fnb):
            ...
            out(1, '===============', i+1, '/', fnb,'===============|')
            out(1, 'Working on', f)
            mofpar = _3_fitmof.fitmof(i, self.data, self.params, 
                              savedir=self.as_exe*'./' or (not self.as_exe)*'results/')[0]
            mpar += [mofpar.tolist()]
    ...
    def fitnum(self):
        ...
        for i in range(fnb):
            ...
            _4b_fitnum.fitnum(i, self.data, self.params, 
                              savedir=self.as_exe*'./' or (not self.as_exe)*'results/')
    ...
    def get_src_par(self):
        par = np.array(self.params['SRC_PARAMS'])
    ...
    def deconvolve(self):
        ...
        self.params['IMG_OFFSETS'] = offsets.tolist()
