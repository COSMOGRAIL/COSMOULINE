...
class MCS_interface():
    ...
    def __init__(self, cfg_file='config.py', standalone=False):
        self.cfg = cfg_file
    ...
    def check_workspace(self):
        ...
        while not self.check_cfg_val():
            ...
            _ = raw_input()
        files, cat = ws.get_files(FILENAME, directory='images', standalone=self.standalone)
    ...
    def set_up_workspace(self, extract=True, clear=False, backup=False):
        ...
        if extract:
            ...
            ws.populate(self.params['FILENAME'], self.params['STARS'], self.params['NPIX'], 
                        ...
                        self.params['SIGMA_SKY'], self.params['IMG_GAIN'], clean = 0, standalone=self.standalone)
