...
import os, mmap, marshal, struct, pickle
...
def bench():
    ...
    def f1():
        return builtin_map(lambda x: pow(x, 10 ** 1000, 10 ** 9), list(range(10 ** 3)))

    def g1():
        return [pow(x, 10 ** 1000, 10 ** 9) for x in range(10 ** 3)]

    def f2():
        return builtin_map(lambda x: x ** 2, list(range(10 ** 6)))
...
def test():
    print('Testing:')
    assert [x ** 2 for x in range(10 ** 4)] == [x ** 2 for x in range(10 ** 4)]
    assert [x ** 2 for x in range(10 ** 4)] == list(map(lambda x: x ** 2, list(range(10 ** 4)), n=10))
    assert [x ** 2 for x in range(10 ** 4)] == list(map(lambda x: x ** 2, list(range(10 ** 4)), n=1))
    assert [(x ** 2,) for x in range(10 ** 3, 10 ** 4)] == [(x ** 2,) for x in range(10 ** 3, 10 ** 4)]
    assert [(x ** 2,) for x in range(10 ** 3, 10 ** 4)] == list(
        map(lambda x: (x ** 2,), list(range(10 ** 3, 10 ** 4)), n=10))
    assert [(x ** 2,) for x in range(10 ** 3, 10 ** 4)] == list(
        map(lambda x: (x ** 2,), list(range(10 ** 3, 10 ** 4)), n=1))
    assert builtin_map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2))) == list(
        map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2))))
    assert builtin_map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2))) == list(
        map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2)), n=10))
    assert builtin_map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2))) == list(
        map(lambda x, y: x + 2 * y, list(range(100)), list(range(0, 200, 2)), n=2))
    # Some Windows (Cygwin) boxes can't fork more than about 15 times, so only test to n=15
    for n in range(1, 15):
        assert [x ** 3 for x in range(200)] == list(map(lambda x: x ** 3, list(range(200)), n=n))
    ...
    check_raises(lambda: list(map(f, [1, 0], n=2)), KeyError)
    check_raises(lambda: list(map(f, [0, 1], n=2)), KeyError)
    check_raises(lambda: list(map(f, [1, 0, 0], n=3)), KeyError)
    check_raises(lambda: list(map(f, [0, 1, 0], n=3)), KeyError)
    check_raises(lambda: list(map(f, [0, 0, 1], n=3)), KeyError)
