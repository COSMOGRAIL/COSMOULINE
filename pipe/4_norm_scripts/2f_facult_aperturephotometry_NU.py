...
colors = ['crimson', 'chartreuse', 'purple', 'cyan', 'gold', 'black', 'blue', 'magenta', 'brown', 'green', 'silver', 'yellow']
	...
	for ind, mjd in enumerate(roundmjds):
		...
		for indn, normcoeffstars in enumerate(normcoeffstarslist):
			...
			for ind, s in enumerate([s for s in photomstars if s.name in starstoplot]):
				...
				lcs.append(lc)
				plt.annotate("Error on the mean : +/- %.1f mmag\n Median absolute deviation : +/- %.1f mmag" % (float(yerrstd[0][0]*1000),  float(np.median(np.fabs(lc["magnitude"] - np.median(lc["magnitude"])))*1000)), xy=(0.2, 1.0), xycoords='axes fraction',
	...
	for indn, normcoeffstars in enumerate(normcoeffstarslist):
		...
		for ind, s in enumerate([s for s in photomstars if s.name in starstoplot]):
			...
			for rmjd in roundmjds:
				...
				medmags.append(np.median([mag for (mag, mymjd) in zip(lc["magnitude"], lc["mjds"]) if round(mymjd) == rmjd]))
				medmagerrtops.append(np.median([mag for (mag, mymjd) in zip(lc["magerrtop"], lc["mjds"]) if round(mymjd) == rmjd]) / np.sqrt(len([mag for (mag, mymjd) in zip(lc["magerrtop"], lc["mjds"]) if round(mymjd) == rmjd])))
				medmagerrbottoms.append(np.median([mag for (mag, mymjd) in zip(lc["magerrbottom"], lc["mjds"]) if round(mymjd) == rmjd]) / np.sqrt(len([mag for (mag, mymjd) in zip(lc["magerrtop"], lc["mjds"]) if round(mymjd) == rmjd])))


			plt.subplot(len(starstoplot), len(normcoeffstarslist), ind* len(normcoeffstarslist)+(indn+1))
			plt.errorbar(medmjds, medmags, yerr=[medmagerrtops, medmagerrbottoms], label=s.name, color=colors[ind], linewidth=2, fmt='o')

			plt.axis([57432, 57450, min(lc["magnitude"])+1.2*max(lc["magerrbottom"]), max(lc["magnitude"])-1.2*max(lc["magerrtop"])])
