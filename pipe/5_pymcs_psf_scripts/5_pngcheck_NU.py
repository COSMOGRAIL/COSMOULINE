...
print(
    "Something very special here : I do already look at the psfskiplist, so that I don't crash even if some of your pyMCS PSFs are broken.")
...
for i, image in enumerate(images):
    ...
    if os.path.isfile(goodpsfstar_filename):
        goodpsfstars = star.readmancat(goodpsfstar_filename)
    else:
        goodpsfstars = psfstars

    nbrpsf = len(goodpsfstars)
    ...
    for j in range(nbrpsf):
        ...
        index = [ind for ind, s in enumerate(psfstars) if goodpsfstars[j].name == s.name][0]
        if hasattr(psfstars[index], "reg"):
            for circle in psfstars[index].reg.circles:
                f2nimg.drawcircle(circle["x"], circle["y"], r=circle["r"], colour=(170))
        f2nimg.writetitle("%s" % (goodpsfstars[j].name))
...
if update:  # remove all the symlink and redo it again with the new images
    allimages = db.select(imgdb, ['gogogo', 'treatme', psfkeyflag], [True, True, True], returnType='dict',
                          sortFields=['setname', 'mjd'])
