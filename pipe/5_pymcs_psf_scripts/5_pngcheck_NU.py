...
if sys.path[0]:
    ...
    # will be emtpy.
    sys.path.append('..')
from config import configdir, settings, psfstarcat, psfkeyflag, imgdb,\
                   psfkicklist, psfkey, psfdir
...
if settings['update']:
    ...
    print("Or recreate it if you deleted it to save space")
...
print("Something very special here : I do already look at the psfskiplist")
print("so that I don't crash even if some of your pyMCS PSFs are broken.")
if os.path.exists(psfkicklist):
    # image[1] would be the comment
    psfskipimages = [image[0] for image in readimagelist(psfkicklist)] 
    print("It contains %i images." % len(psfskipimages))

    images = [image for image in images 
                     if image["imgname"] not in psfskipimages]
else:
    print("No skiplist.")
...
for i, image in enumerate(images):
    ...
    for j in range(nbrpsf):
        ...
        starcosmicspklfilepath = os.path.join(resultsdir, 
                                              "starcosmics_%03i.pkl" % (j+1))
