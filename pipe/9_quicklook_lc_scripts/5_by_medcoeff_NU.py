...
for s in ptsources:
	...
	plt.scatter(mhjds, mags, s=12, c=medcoeffs, vmin=0.7,vmax=2.0, edgecolors='none')
...
if savefigs:
	if plotnormfieldname:
		plotfilepath = os.path.join(plotdir, "%s_lc_%s_by_medcoeff.pdf" % (deckey, plotnormfieldname))
	else :
		plotfilepath = os.path.join(plotdir, "%s_lc_by_medcoeff.pdf" % (deckey))
