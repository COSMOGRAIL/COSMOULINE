...
deconvstarscatpath = os.path.join(configdir, "deconvstars.cat")
deconvstars = star.readmancat(deconvstarscatpath)

print "I will run a full deconvolution for the following stars:"
print [star.name for star in deconvstars]
proquest(True)

if thisisatest == False:
    print "Warning : This is NOT a test !"
    print "You did not forget to update your deconvstars.cat with only the good stars, right ?"
    proquest(True)
