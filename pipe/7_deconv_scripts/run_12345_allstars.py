...
for star in photomstars :
    ...
    if computer == "regor4" :
		if not os.path.exists(visudir + "/dec_" + star.name): 
			os.makedirs(visudir + "/dec_" + star.name)
			print visudir + "/dec_" + star.name
		
		print "I'll copy the png to your visudir !"
		key = "dec_" + decname + "_" + star.name + "_" + decnormfieldname + "_" + "_".join(decpsfnames)
		os.system("cp "+workdir+"/"+key+ "_png/"+"0*.png " + visudir + "/dec_" + star.name)
