...
if not settings['checkplots']:
    ...
    if "seeing" not in db.getFieldNames(imgdb):
        db.addFields(imgdb, ['seeing:float', 'ell:float', 
                             'goodstars:int', 'seeingpixels:float', 
                             'pa:float'])
...
for i, image in enumerate(images):
    ...
    if len(fwhms) > 10:
        ...
        if maxpos == 0:
            ...
            print("This image if funny,",
                  "it seems to have many high-FWHM objects.")
            ...
            peakpos - 2.0, peakpos + 2.0))
            maxpos = np.argmax(hist)
            peakpos = 0.5 * (edges[maxpos] + edges[maxpos + 1])

            # We take the median of values around this peakpos :
            starfwhms = fwhms[np.logical_and(fwhms > peakpos - 1.0, 
                                             fwhms < peakpos + 1.0)]
            if len(starfwhms) > 0:
        ...
        print(f"Only {len(fwhms)} stars, using the median ...")
        seeingpixels = np.median(fwhms)
        if forceseeingpixels:
            if seeingpixels < 2:
                seeingpixels = 2.01  # HAAAAAAX
    ...
    if settings['checkplots']:
        ...
        plt.hist(fwhms, bins=np.linspace(np.min(fwhms), np.max(fwhms), 50),
                        facecolor='green')
    ...
    # And we measure the ellipticity of the images, 
    # by looking at sources with similar width then our seeingpixels
    # Now look at this beauty : :-)
    ells = np.array([s.ell for s in sortedsexstars])
    starells = np.array([s.ell for s in sortedsexstars 
                                   if abs(s.fwhm - seeingpixels) < 1.0])
    ...
    starpas = np.array([s.props["THETA_IMAGE"] for s in sortedsexstars 
                                       if abs(s.fwhm - seeingpixels) < 1.0])
    ...
    starbimgs = np.array([s.props["B_IMAGE"] for s in sortedsexstars 
                                      if abs(s.fwhm - seeingpixels) < 1.0])

    aimgs = np.array([s.props["A_IMAGE"] for s in sortedsexstars])
    staraimgs = np.array([s.props["A_IMAGE"] for s in sortedsexstars 
                                      if abs(s.fwhm - seeingpixels) < 1.0])
    ...
    if not settings['checkplots']:
        db.update(imgdb, ['recno'], [image['recno']],
                  {'seeing': float(seeing), 'ell': float(ell), 
                   'goodstars': nbrstars,
                   'seeingpixels': float(seeingpixels), 'pa': float(pa), 
                   'pastd': float(pastd), 'bimage': float(bimage),
                   'aimage': float(aimage)})
